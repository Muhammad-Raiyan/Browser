...
libzmq_install() {
    ...
    if [ "${BUILD_TYPE}" = "cmake" ] ; then
        cmake -Hlibzmq-${ZMQ_VERSION} -B${LIBZMQ} -G "Visual Studio 15 2017 Win64" -DWITH_PERF_TOOL=OFF \
...
cppzmq_build() {
    ...
    cmake -H. -B${CPPZMQ} -G "Visual Studio 15 2017 Win64" -DENABLE_DRAFTS=${ENABLE_DRAFTS} \
                          -DCOVERAGE=${COVERAGE}
    cmake --build ${CPPZMQ}
    # -- -j${JOBS}
    popd
